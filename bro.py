...
# -----------------------------
# Ensure admin exists (auto-create default admin/admin123)
# -----------------------------
def ensure_admin_exists():
    try:
        ...
        # if table missing or DB not reachable, log to console (UI will show DB errors later)
...
# -----------------------------
# Logout helper (safe across streamlit versions)
# -----------------------------
def safe_logout_message_and_refresh():
    ...
    # try to rerun (works on many versions), fallback to meta refresh if attribute missing
    try:
...
# -----------------------------
    ...
    if not st.session_state.logged_in or st.session_state.role != "admin":
        ...
        if st.button("Login as Admin"):
            try:
                # authenticate and ensure username is 'admin' (case-insensitive)
