...
def add_column_to_movies(col_name: str, col_type: str, allow_nulls: bool = True):
    ...
    df[col_name] = pd.Series([None]*len(df))
    save_movies(df)
...
def get_recommendations(df, base_title, topn=5):
    if df.empty or not base_title.strip():
        return pd.DataFrame()
    df = df.copy()
    df['combined'] = df.get('genre','').fillna('') + " " + df.get('director','').fillna('')
...
# -----------------------------
    ...
    if st.session_state.logged_in and st.session_state.role=="admin":
        ...
        admin_menu = st.selectbox("Choose action", ["Home","Add Movie","Update Movie","Delete Movie","Add Column","Filter Movies","Recommendations","Logout"])
        df = load_movies()
        if admin_menu=="Home":
            st.dataframe(df,use_container_width=True)
            ...
            dur = st.number_input("Duration",1,1000,120)
